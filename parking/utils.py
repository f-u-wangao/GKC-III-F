...
cameraMatrix = np.array([[489.3828, 0.8764, 297.5558],
                            [0, 489.8446, 230.0774],
                            [0, 0, 1]])
distCoeffs = np.array([-0.4119,0.1709,0,0.0011, 0.018])
...
def dip(img: np.ndarray) -> list:
    ...
    """
    h, w = img.shape[:2]
    newcameramtx, roi = cv.getOptimalNewCameraMatrix(cameraMatrix, distCoeffs,
                                                     (w, h), 1, (w, h))
    ...
    src_points = np.float32([[85, 250], [529, 250], [0, rows - 1],
                             [cols - 1, rows - 1]])
    dst_points = np.float32([[0, 0], [cols - 1, 0], [180, rows - 1],
                             [434, rows - 1]])
